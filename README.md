...
> Не разбираетесь в Docker/n8n? Читайте `docs/ПРОСТАЯ-ИНСТРУКЦИЯ.md` — пошаговая инструкция для обычного пользователя (что установить, какие команды написать, как перенести бота на другой компьютер).
