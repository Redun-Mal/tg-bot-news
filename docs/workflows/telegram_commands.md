...
**Quirk 10** (also from real user feedback): `/sources` errored while `/help` and `/add_source` worked, because its path runs a `Format sources list` aggregation step (N Postgres rows -> 1 summary message) before `Prepare reply`, which broke the `$('Parse command').item.json.chatId` pairedItem lookup that the _other_ branches happened not to exercise. Fixed by using `$('Parse command').first().json.chatId` instead — safe here since exactly one Telegram message triggers exactly one execution, so `.first()` and `.item` are equivalent whenever the pairedItem chain hasn't been broken, but only `.first()` survives when it has. See `docs/decisions/005-n8n-postgres-node-quirks.md`.

**Not an n8n bug, also from user feedback**: `/help`'s line breaks initially showed as literal `\n` text instead of real newlines — a Python string-escaping mistake in the build tooling (wrote `"\\n"` instead of a real newline character when generating the `Set` node's value), not an n8n limitation. See `docs/decisions/005-n8n-postgres-node-quirks.md` for why `/sources` was unaffected by the same class of mistake.
