...
`n8n/workflows/classify_with_claude.json` is **verified for the filter and graceful-Claude-failure paths**: a giveaway-pattern post correctly filtered out and turned into a synthetic, already-resolved `news_item` without ever reaching Claude; a normal post correctly passed the filter, had its request built, and — with no real Claude API key available — got a genuine `401 authentication_error: invalid x-api-key` from Anthropic's real API (confirmed via the raw error body, not a parser or network failure), correctly caught by `onError: continueErrorOutput` and logged, leaving the post unresolved for retry. This surfaced Quirk 6: embedding the classification tool's full JSON schema directly inside the `={{ }}` expression building the request body made n8n's expression parser fail with `"invalid syntax"` — misleadingly easy to mistake for an HTTP/auth error since `onError` handling looks identical either way. Fixed by moving body construction into a preceding Code node and referencing its output with a simple `={{ $json.body }}`. See `docs/decisions/005-n8n-postgres-node-quirks.md`. The **successful-classification path** (parsing a real Claude response, level-4 dedup, creating a real `news_items` row) is unverified — needs a real `CLAUDE_API_KEY`.
